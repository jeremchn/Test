...
GPT_KEY =  "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"
...
def enrich_with_news_and_revenue(df, news_api_key, revenue_api_key):
    ...
    try:
        ...
        data = {
            "model": "gpt-4-turbo",  # Utilisez "gpt-4" ou "gpt-4-32k" si nécessaire
...
def generer_actualites_top_entreprises(df, top_n=5):
    ...
    for entreprise in top_entreprises:
        prompt = f"""
You are a finance and corporate news expert. Your role is to provide an important (and dated) news item regarding the company "{entreprise}".

Use the most accurate data from your knowledge base. You MUST provide a response, even if the company is little-known. Search your historical knowledge up to 2024 or later if available.
...
{{
  ...
  "actualité": "A clear and concise sentence in English summarizing a major or recent event for this company"
}}

⚠️ Do not provide any commentary, explanation, or text before or after. Return ONLY the JSON, even if you are uncertain.
"""
...
def main(input_csv_path, output_csv_path, output_json_path, contacts_csv_path, contacts_json_path):
    ...
    df = pd.read_csv(input_csv_path, on_bad_lines='skip', delimiter='|')
